...
def get_connection() :
    conn = pymysql.connect(host='127.0.0.10', user='root',
            password='1234', db='flaskDb', charset='utf8')
    if conn:
        print('디비 접속 완료')
    return conn
...
def get_member_list() :
    ...
    sql = ''' SELECT * FROM memberTBL ORDER BY userNo DESC  '''
    cursor.execute(sql)
...
def member(no) :
    ...
    sql = '''SELECT * FROM memberTBL where userNo = %s'''
    cursor.execute(sql, no)
    result = cursor.fetchone()
    if result:
        temp_dic = {}
...
def member_update(c_no, c_id, c_pwd):
    ...
    cursor = conn.cursor()
    # 레코드 수정 sql 구문 
    sql = '''update membertbl set userID=%s,pwd=%s where userNo=%s'''
